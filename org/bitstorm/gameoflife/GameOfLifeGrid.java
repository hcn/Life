...
public class GameOfLifeGrid implements CellGrid {
  ...
  public synchronized void next() {
    ...
    int col, row;
    int neighbours;
    Enumeration<Cell> fenum;

    generations++;
    nextShape.clear();

    // Reset cells
    fenum = currentShape.keys();
    while ( fenum.hasMoreElements() ) {
      cell = (Cell) fenum.nextElement();
      cell.neighbour = 0;
    }
    // Add neighbours
    ...
    // Bury the dead
    // We are walking through an fenum from we are also removing elements. Can be tricky.
    fenum = currentShape.keys();
    while ( fenum.hasMoreElements() ) {
      cell = (Cell) fenum.nextElement();
  ...
  /**
   * Get fenumeration of Cell's
   * @see org.bitstorm.gameoflife.CellGrid#getfenum()
   */
